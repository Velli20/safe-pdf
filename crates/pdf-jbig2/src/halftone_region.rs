...
/// Decoded JBIG2 halftone-region segment.
///
/// ITU-T T.88 / ISO/IEC 14492 section 7.4.5 defines halftone-region segments
/// as a bitmap region produced from a referred pattern dictionary and a
/// decoded gray-scale image. The common [`RegionInfo`] fields preserve where
/// section 8.2 page composition places that decoded bitmap.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct DecodedHalftoneRegionSegment {
    /// Decoded halftone bitmap generated by the section 6.6 halftone procedure.
    pub(crate) image: JBig2Image,
    /// Region placement and page-composition metadata from section 7.4.1.
    pub(crate) region: RegionInfo,
}

impl DecodedHalftoneRegionSegment {
    /// Compose the decoded halftone bitmap into `dst` at the region position.
    ///
    /// JBIG2 T.88 / ISO/IEC 14492 section 7.4.5 defines the halftone-region
    /// segment, and section 6.6.5 places the decoded pattern cells into the
    /// region bitmap using the region's composition operator. This helper
    /// forwards that final page-image composition step to
    /// [`JBig2Image::compose_clipped_to`].
...
pub(crate) fn decode_halftone_region_segment(
...
) -> Result<DecodedHalftoneRegionSegment, Jbig2Error> {
    ...
    Ok(DecodedHalftoneRegionSegment {
        image: region_image,
...
mod tests {
    ...
    use crate::{
        error::Jbig2Error, halftone_region::DecodedHalftoneRegionSegment, image::JBig2Image,
        region_info::RegionInfo,
    };
    use pdf_utils::BitReader;
    ...
    fn compose_clipped_to_places_halftone_bitmap_using_region_metadata() {
        ...
        let decoded = DecodedHalftoneRegionSegment {
            image,
            region: RegionInfo {
                width: 1,
                height: 1,
